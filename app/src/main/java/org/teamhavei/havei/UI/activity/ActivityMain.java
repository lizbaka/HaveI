...
import org.teamhavei.havei.service.HaveITimeWatcher;
...
public class ActivityMain extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        iconManager = new IconManager(ActivityMain.this);
    ...
    private void initNotificationChannel() {
        NotificationChannel basicChannel = new NotificationChannel(Util.BASIC_NOTIFICATION_CHANNEL_ID, getString(R.string.basic_notification_channel_name), NotificationManager.IMPORTANCE_MIN);
        NotificationChannel todoChannel = new NotificationChannel(Util.TODO_NOTIFICATION_CHANNEL_ID, getString(R.string.todo_reminder_notification_channel_name), NotificationManager.IMPORTANCE_DEFAULT);
        NotificationChannel habitChannel = new NotificationChannel(Util.HABIT_NOTIFICATION_CHANNEL_ID, getString(R.string.habit_reminder_notification_channel_name), NotificationManager.IMPORTANCE_DEFAULT);
    ...
    private void initBasicPart() {
        ...
        pref = getSharedPreferences(Util.PREF_SETTINGS, MODE_PRIVATE);
        if (pref.getBoolean(Util.PREF_FIRST_RUN, true)) {
            firstRun();
            pref.edit().putBoolean(Util.PREF_FIRST_RUN, false)
                    .apply();
        }
        if (pref.getLong(Util.PREF_FIRST_RUN_DATE, -1) == -1) {
            pref.edit().putLong(Util.PREF_FIRST_RUN_DATE, Calendar.getInstance().getTimeInMillis())
                    .apply();
        }
    }
    ...
    void configGreetingCard() {
        ...
        greetingTimeTV.setText(Util.getGreetingTimeId());
        greetingSecTV.setText(Util.getGreetingSecId());
        greetingIconIV.setImageResource(Util.getGreetingIconId());
    }
    ...
    private void configTodoCard() {
        ...
        if (todoList != null && !todoList.isEmpty()) {
            ...
            todoIconIV.setImageDrawable(iconManager.getIcon(eventDBHelper.findEventTagById(todo.getTagId()).getIconId()));
    ...
    private void configHabitCard() {
        ...
        if (habitList != null && !habitList.isEmpty()) {
            ...
            for (int i = 0; i < (Math.min(habitList.size(), 4)); i++) {
                ...
                iconIV.setImageDrawable(iconManager.getIcon(eventDBHelper.findEventTagById(mHabit.getTagId()).getIconId()));
    ...
    private void configBookkeepCard() {
        ...
        double budget = pref.getFloat(Util.PREF_BUDGET, 0);
    ...
    private void firstRun() {
        ...
        firstTodo.setDateTime(Util.eventDatetimeFormatter(new Date()));
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            ...
                            urlBuilder.addQueryParameter("date", Util.eventDateFormatter(goal.getTime()));
                            Request request = new Request.Builder()
                                    .url(urlBuilder.build()).build();
                            client.newCall(request).enqueue(new Callback() {
                                @Override
