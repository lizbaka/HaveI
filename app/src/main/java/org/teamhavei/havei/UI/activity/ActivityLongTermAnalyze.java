...
public class ActivityLongTermAnalyze extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        firstRun.setTimeInMillis(pref.getLong(Util.PREF_FIRST_RUN_DATE, firstRun.getTimeInMillis()));
    ...
    private void configStatisticsCard() {
        ...
        if (bookkeepCount <= 0) {
            ...
            List<BookTag> bookTag = bookkeepDBHelper.findAllBookTagSortByPM(Util.BOOKKEEP_TAG_INCOME);
            if (bookTag.isEmpty()) {
                mostInBookTagLL.setVisibility(View.GONE);
            } else {
                mostInBookTagLL.setVisibility(View.VISIBLE);
            ...
            bookTag = bookkeepDBHelper.findAllBookTagSortByPM(Util.BOOKKEEP_TAG_EXPENDITURE);
            if (bookTag.isEmpty()) {
                mostExBookTagLL.setVisibility(View.GONE);
            } else {
                mostExBookTagLL.setVisibility(View.VISIBLE);
    ...
    private void initView() {
        ...
        ((TextView) findViewById(R.id.long_term_first_date)).setText(Util.eventDateFormatter(firstRun.getTime()));
    ...
    public void setBookkeepBAR(BarChart mBar) {
        ...
        for (Long datel : mHash.keySet()) {
            xValues.add(Util.eventYearMonthFormatter(new Date(datel)));
        }
        int i = 0;
