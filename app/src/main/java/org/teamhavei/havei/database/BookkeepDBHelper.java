package org.teamhavei.havei.database;
...
public class BookkeepDBHelper extends SQLiteOpenHelper {
    ...
    public double getIncomeByMonth(Date yearMonth) {
        String sYearMonth = Util.eventYearMonthFormatter(yearMonth);
    ...
    public double getExpenditureByDay(Date date) {
        String sDate = Util.eventDateFormatter(date);
    ...
    public double getPMBeforeYearMonthByAccount(int accountId, Date date) {
        String yearMonth = Util.eventYearMonthFormatter(date);
    ...
    public List<Bookkeep> findBookkeepByMonth(Date date) {
        String sDate = Util.eventYearMonthFormatter(date);
    ...
    public ArrayList<Double> getPMListByYear(int type, Date date) {
        ...
        if (type == Util.BOOKKEEP_TAG_EXPENDITURE) {
            for (int i = 0; i < 12; i++) {
    ...
    public ArrayList<Double> getSurplusListByYear(Date date) {
        ArrayList<Double> surplusData = getPMListByYear(Util.BOOKKEEP_TAG_INCOME, date);
        ArrayList<Double> expenditureData = getPMListByYear(Util.BOOKKEEP_TAG_EXPENDITURE, date);
    ...
    public List<BookTag> findAllBookTag(boolean excludeDeleted) {
        ...
        if (cursor.getCount() <= 0) {
            ...
            defaultBookTag.setIconId(IconManager.DEFAULT_BOOKKEEP_TAG_ICON_ID);
            defaultBookTag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
            ...
            defaultBookTag.setType(Util.BOOKKEEP_TAG_INCOME);
    ...
    public void deleteBookAccount(int accountId) {
        if (accountId == Util.DEFAULT_ACCOUNT_ID) {
            return;
        }
    ...
    public void initializeTag() {
        ...
        account.setId(Util.DEFAULT_ACCOUNT_ID);
        account.setName(mContext.getString(R.string.default_account));
        account.setIconId(IconManager.DEFAULT_ACCOUNT_ICON_ID);
        account.setInit(0);
        ContentValues values = bookAccountToValues(account);
        values.put(BOOK_ACCOUNT_ID, Util.DEFAULT_ACCOUNT_ID);
        ...
        tag.setIconId(IconManager.DEFAULT_BOOKKEEP_TAG_ICON_ID);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName(mContext.getString(R.string.default_income_tag));
        tag.setIconId(IconManager.DEFAULT_BOOKKEEP_TAG_ICON_ID);
        tag.setType(Util.BOOKKEEP_TAG_INCOME);
        insertBookTag(tag);
        tag.setName("饮食");
        tag.setIconId(IconManager.ID_CS_STAPLE_FOOD);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("零食");
        tag.setIconId(IconManager.ID_CS_SNACKS);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("交通");
        tag.setIconId(IconManager.ID_CS_CAR_FARE);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("薪资");
        tag.setIconId(IconManager.ID_CS_SALARY);
        tag.setType(Util.BOOKKEEP_TAG_INCOME);
        insertBookTag(tag);
        tag.setName("奖金");
        tag.setIconId(IconManager.ID_CS_BONUS);
        tag.setType(Util.BOOKKEEP_TAG_INCOME);
        insertBookTag(tag);
        tag.setName("红包");
        tag.setIconId(IconManager.ID_CS_RED_PACKET);
        tag.setType(Util.BOOKKEEP_TAG_INCOME);
        insertBookTag(tag);
        tag.setName("收益");
        tag.setIconId(IconManager.ID_CS_LEND_OUT);
        tag.setType(Util.BOOKKEEP_TAG_INCOME);
        insertBookTag(tag);
        tag.setName("日用");
        tag.setIconId(IconManager.ID_CS_LAUNDRY_DETERGENT);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("住宿");
        tag.setIconId(IconManager.ID_CS_ACCOMMODATION);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("差旅");
        tag.setIconId(IconManager.ID_CS_AIR_TICKETS);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("衣物");
        tag.setIconId(IconManager.ID_CS_CLOTHING);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("娱乐");
        tag.setIconId(IconManager.ID_CS_GAME);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("锻炼");
        tag.setIconId(IconManager.ID_CS_EXERCISE);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("爱好");
        tag.setIconId(IconManager.ID_CS_HOBBY);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("医疗");
        tag.setIconId(IconManager.ID_CS_MEDICINE);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("宠物");
        tag.setIconId(IconManager.ID_CS_PETS);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
        tag.setName("学习");
        tag.setIconId(IconManager.ID_CS_TEXTBOOK);
        tag.setType(Util.BOOKKEEP_TAG_EXPENDITURE);
        insertBookTag(tag);
    }
}
