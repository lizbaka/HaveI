...
public class EventDBHelper extends SQLiteOpenHelper {
    ...
    public List<Habit> findHabitByReminderTime(Date time) {
        String sTime = Util.eventTimeFormatter(time);
    ...
    public List<Habit> findAllHabitOrderByRank() {
        ...
        for (int i = 0; i < habitList.size(); i++) {
            ...
            if (thisExecList.size() <= 0) {
                ...
                firstExecDate.setTime(Util.eventDateParser(thisExecList.get(0).getDate()));
    ...
    public List<HabitExec> findHabitExecByHabitIdWithDateRange(int habitId, Date startDate, Date endDate) {
        String sStartDate = Util.eventDateFormatter(startDate);
        String sEndDate = Util.eventDateFormatter(endDate);
    ...
    public List<HabitExec> findHabitExecByHabitIdWithYearMonth(int HabitId, Date yearMonth) {
        String sYearMonth = Util.eventDateFormatter(yearMonth).substring(0, 7);
    ...
    public boolean isHabitDoneToday(int habitId) {
        Cursor cursor = db.query(TABLE_HABIT_EXECS, null, HABIT_EXECS_HABIT_ID + " = ?" + " AND " + HABIT_EXECS_DATE + " = ?", new String[]{Integer.toString(habitId), Util.eventDateFormatter(new Date())}, null, null, null);
    ...
    public TreeMap<Long, Integer> getHabitExecCountListMonthly(Date since) {
        ...
        for (Calendar today = Calendar.getInstance(); calendarSince.get(Calendar.MONTH) <= today.get(Calendar.MONTH); calendarSince.add(Calendar.MONTH, 1)) {
            Cursor cursor = db.query(TABLE_HABIT_EXECS, null, HABIT_EXECS_DATE + " LIKE ?", new String[]{Util.eventYearMonthFormatter(calendarSince.getTime()) + "%"}, null, null, null);
    ...
    public List<EventTag> findAllEventTag(Boolean excludeDeleted) {
        ...
        if (cursor.getCount() <= 0) {
            ...
            defaultEventTag.setIconId(IconManager.ID_HS_RECORD);
    ...
    public List<Todo> findTodoByDatetime(Date datetime) {
        String sDatetime = Util.eventDatetimeFormatter(datetime);
    ...
    public List<Todo> findTodoAfterToday(boolean excludeDone) {
        String sNow = Util.eventDateFormatter(new Date());
        Cursor cursor;
        if (excludeDone) {
    ...
    public void initializeTag() {
        ...
        tag.setName("默认分类");
        tag.setIconId(IconManager.DEFAULT_EVENT_TAG_ICON_ID);
        insertEventTag(tag);
        tag.setName("锻炼");
        tag.setIconId(IconManager.ID_HS_EXERCISE);
        insertEventTag(tag);
        tag.setName("学习");
        tag.setIconId(IconManager.ID_HS_BOOKS);
        insertEventTag(tag);
        tag.setName("饮食");
        tag.setIconId(IconManager.ID_HS_EAT);
        insertEventTag(tag);
        tag.setName("娱乐");
        tag.setIconId(IconManager.ID_HS_GAME_MACHINE);
        insertEventTag(tag);
        tag.setName("财富");
        tag.setIconId(IconManager.ID_HS_MONEY);
        insertEventTag(tag);
    }
}